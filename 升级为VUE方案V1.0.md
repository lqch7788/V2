...
> 日期: 2026-05-20 | 更新: 2026-05-20（基于 iAGS 原系统全量代码分析修正）

**⚠️ 重要修正说明**：本文档初次编写时基于对 iAGS 系统的推测，现已基于 `D:\iAGS\` 源码全量分析进行了修正。关键修正：BizServer 端口 8088（非 3000）、RSA 动态公钥加密登录、Cool 主题配色规格、V1.1 延迟整合策略。
...
| **iAGS 后端** | **保持不变**（BizServer 端口 8088，broker API + MySQL + Cookie 认证） | 不动 BizServer/Foil/YuJiang 框架 |
| **V1.1 前端** | React/TSX → Vue 3 SFC 全部重写，模块迁入新项目（**等 iAGS 16 模块改造完后再整合**） | React 代码全部废弃 |
...
> **实际 iAGS 认证流**：登录页 EJS 由服务端注入 RSA 公钥（NodeRSA, pkcs1+pkcs8-public-pem+base64），客户端加密密码后以 form-urlencoded POST 到 `/app/account/login`，服务端 Set-Cookie `yujiang-iAGS-x-access-token`（httpOnly, 8h 有效期）。

```typescript
// src/stores/useAuthStore.ts — iAGS Cookie 认证对接
import { defineStore } from 'pinia'
import { ref, computed } from 'vue'
import { brokerGet } from '@/services/brokerClient'

interface User {
  OID: string; AID: string; Name: string; OrgOID: string
  isAdmin: boolean
  org?: { isRoot: boolean; data: { OrgAID: string; Name: string; VI_Name?: string } }
}

interface ProcessNode {
  processOID: string; processAID: string; name: string
  route?: string; icon?: string; children: ProcessNode[]
  authority: Record<string, number>; isAuthorized: boolean
}
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(userAID: string, password: string, rememberMe = false) {
    // 注意：生产环境需先从服务端获取 RSA 公钥，加密密码后提交
    // 公钥由服务端动态注入登录页 EJS 模板：var publicKey=<%-JSON.stringify(locals.publicKey)%>
    // 加密参数：NodeRSA, encryptionScheme: 'pkcs1', key format: 'pkcs8-public-pem', output: 'base64'
    const res = await fetch('/app/account/login', {
      ...
      body: new URLSearchParams({
        userAID,
        password,           // 生产环境应为 RSA 加密后的 encryptedPassword
        loginType: 'userID',
    ...
    if (data.OID || data.AID) {
      user.value = { OID: data.OID || data.AID, AID: data.AID || data.OID,
        Name: data.Name || userAID, OrgOID: data.OrgOID || '', isAdmin: data.isAdmin || false,
        org: data.org }
    }
    if (rememberMe) {
  ...
  async function logout() {
    ...
    user.value = null; processTree.value = []
  }

  async function fetchUserInfo() {
    const result = await brokerGet<User>(['account', 'getMyInfo'])
    if (result && (result as unknown as User).OID) user.value = result as unknown as User
  }

  async function fetchProcessTree() {
    const result = await brokerGet<{ children: ProcessNode[] }>(['org', 'getUserProcessTree'])
    processTree.value = (result as unknown as { children: ProcessNode[] })?.children || []
  }
  ...
  async function checkAuth(): Promise<boolean> {
    ...
    try {
      ...
      if ((result as unknown as { valid: boolean })?.valid) { await fetchUserInfo(); await fetchProcessTree(); isLoading.value = false; return true }
    } catch { /* Cookie 无效 */ }
    isLoading.value = false; return false
  }

  return { user, processTree, isLoading, isAuthenticated, login, logout, fetchUserInfo, fetchProcessTree, checkAuth }
})
```
...
- [ ] 配置 Vite proxy (`/app` → `localhost:8088`, `/socket.io` → ws)
...
# 样式 (Tailwind CSS v3 + PostCSS，不是 v4 的 @tailwindcss/vite)
npm install -D tailwindcss postcss autoprefixer
# 同时创建 postcss.config.js 和 tailwind.config.js 配置文件
...
为什么可行？
├── Vite proxy 让前端 (localhost:5173) 和后端 (localhost:8088)
...
┌─────────────────────────────────────────────────────────┐
│                     Nginx (反向代理)                      │
│  ├── /app/*          → localhost:8088 (iAGS BizServer)   │
│  ├── /socket.io/*    → localhost:8088 (WebSocket)        │
...
└─────────────────────────────────────────────────────────┘
   ...
   (Vite build)      (端口8088,不动)     (TCP/MQTT,不动)
                     - Foil 框架          - IoT 数据采集
                     - broker 接口        - Redis 写入
                     - MySQL 操作
                     - Cookie 认证
...
### 24.2 Vite + PostCSS + Tailwind 配置

**postcss.config.js**（Tailwind v3 使用 PostCSS 插件，非 v4 的 Vite 插件）:
```javascript
// postcss.config.js
export default {
  plugins: {
...
**vite.config.ts** — Vite 代理配置（注意：Tailwind v3 不需要在 plugins 中添加 `tailwindcss()`）:
```typescript
...
export default defineConfig({
  plugins: [vue()],  // Tailwind v3 走 PostCSS，不在此处配置
...
## 附录 D: iAGS Cool 主题视觉效果规格

> 基于原 iAGS 系统 (`D:\iAGS\`) theme Cool + style Cool + project color green 实测提取。
> 新 Vue 3 前端必须精确复现以下视觉效果，**不允许自行改动**。

### D.1 全局布局尺寸

| 区域 | 规格 |
|------|------|
| 顶部 Header | 高度 65px，渐变背景 `linear-gradient(to bottom, #3dcaed, #33b1d0)` |
| 左侧 Sidebar | 宽度 250px，背景 `rgba(50, 66, 78, 1)` |
| 侧边栏右边框 | `2px solid rgb(82, 165, 218)` |
| 底部 Footer | 高度 55px，背景 `#2d3b46` |
| 主体背景 | `#e9eff3` |
| 主内容区 | flex: 1，背景 `#e9eff3` |

### D.2 顶部 Header 样式

```
Logo 区 (左侧):
├── Logo 图标: 48×48px，原路径 /images/LOGO.png（Vue 3 改用内联 SVG data URI）
├── 项目名称(tmCropCloud): 24px Arial bold, 颜色 #fff
├── 中文名: 22px, 字间距 3px, 颜色 #fff
├── 描述文字: 13px, 颜色 #fff
└── 状态标记: 18px red（测试环境显示，正式环境隐藏）

菜单 Tab (右侧):
├── 字体: 18px Microsoft YaHei
├── 默认状态: color rgba(255,255,255,0.85), 无背景
├── Hover: color #fff
├── Active: color #fff, font-weight 600, 背景渐变 linear-gradient(to bottom, #38bee0, #1c71da)
└── Tab 间距: 4px, 内边距 15px 12px

用户区 (最右侧):
├── 用户信息文字: 13px, color #fff
└── 退出按钮: 背景 rgba(255,255,255,0.15), 边框 rgba(255,255,255,0.3), 圆角 3px
```
...
层级缩进:
├── 一级节点: padding-left 0 (默认)
├── 二级节点: padding-left 30px
├── 三级节点: padding-left 40px
├── 四级节点: padding-left 50px
└── 节点内边距: padding 9px 0
...
```
背景: #2d3b46
文字颜色: #888 (默认), #aab (链接), #fff (链接 hover)
分隔符: #555
字体大小: 12px, 居中对齐
```

### D.5 登录页样式（原 iAGS `/app/account/showLogin`）

```
登录卡片:
├── 背景: 白色 rgba(255,255,255,0.9)
├── 顶部渐变条: 蓝色渐变（与 Header 同色系）
├── 圆角: 8px
...
### D.6 EasyUI → Vue 3 主题色映射

| EasyUI 元素 | 原样式 | Vue 3/Element Plus 替代 |
|------------|--------|------------------------|
| 主色调 | `#3dcaed` (Cool 主题蓝) | Element Plus CSS 变量覆盖 |
| 表格选中行 | `#e6f7ff` | `--el-table-current-row-bg-color` |
| 按钮主色 | `#3dcaed` | `--el-color-primary: #3dcaed` |
| 链接色 | `#0b79ff` | `--el-color-primary-light-3` |
| 成功色 | `#27ae60` (green 项目色) | `--el-color-success: #27ae60` |

### D.7 Element Plus 主题覆盖要点

```scss
// 覆盖 Element Plus 默认蓝色为 iAGS Cool 主题色
:root {
  --el-color-primary: #3dcaed;
  --el-color-primary-light-3: #6ad5f2;
  --el-color-primary-light-5: #8ee0f6;
  --el-color-primary-light-7: #b2ebf9;
  --el-color-primary-light-9: #d6f5fc;
  --el-color-primary-dark-2: #33b1d0;

  // 菜单相关
  --el-menu-bg-color: rgba(50, 66, 78, 1);
  --el-menu-text-color: #ccc;
  --el-menu-hover-bg-color: rgba(255, 255, 255, 0.05);
  --el-menu-active-color: #0b79ff;
}
```
...
特殊场景:
├── Logo/英文标识: 'Arial', sans-serif
└── 代码/数据: 'Consolas', 'Courier New', monospace（如有）
```
